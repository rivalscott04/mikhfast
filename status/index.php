...
if (isset($_POST['nama'])) {
	...
	if ($API->connect($iphost, $userhost, decrypt($passwdhost))) {
		...
		$getuser0 = $router->getHotspotUserByName($name);
		$user = $getuser0['name'];
		$profile = $getuser0['profile'];
		$exp = $getuser0['comment'];
		$uptime = formatDTM($getuser0['uptime']);
		$getbytein = $getuser0['bytes-in'];
		$getbyteo = $getuser0['bytes-out'];
		$getbytetot = ($getbytein + $getbyteo);
		$bytetot = formatBytes($getbytetot, 2);
		$limitup = $getuser0['limit-uptime'];
		$limitbyte = $getuser0['limit-bytes-total'];
		if ($limitbyte == "") {
			$dataleft = "Unlimited";
		...
		$getprofile0 = $router->getHotspotUserProfileByName($profile);
		$ponlogin = $getprofile0['on-login'];
