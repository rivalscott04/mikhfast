<?php
$langid="en"; 
$langname = "English";
$language = "Language";

$_about = "About";
$_action = "Action";
$_add = "Add";
$_add_router = "Add Router";
$_add_user = "Add User";
$_add_user_profile = "Add Profile";
$_admin = "Admin";
$_admin_settings = "Admin Settings";
$_all = "All";
$_auto_reload = "Auto load";
$_bluetooth_ac = "Print BT Access Code";
$_board_name = "Board Name";
$_by_comment = "By Comment";
$_cancel = "Cancel";
$_character = "Character";
$_close = "Close";
$_comment = "Comment";
$_confirm = "Confirm";
$_connecting = "Connecting";
$_session_switched = "Switched to: %s";
$_toast_user_added = "User added successfully";
$_toast_user_updated = "User updated successfully";
$_toast_users_generated = "%d voucher(s) generated successfully";
$_toast_profile_added = "Profile added successfully";
$_toast_profile_updated = "Profile updated successfully";
$_toast_user_removed = "User removed successfully";
$_toast_users_removed = "Users removed successfully";
$_toast_user_enabled = "User enabled successfully";
$_toast_user_disabled = "User disabled successfully";
$_toast_user_reset = "User reset successfully";
$_toast_users_removed_comment = "Users removed by comment successfully";
$_toast_expired_removed = "Expired users removed successfully";
$_toast_profile_removed = "Profile removed successfully";
$_toast_settings_saved = "Settings saved successfully";
$_toast_quickprint_saved = "Quick print package saved";
$_toast_quickprint_removed = "Quick print package removed";
$_toast_operation_ok = "Operation completed successfully";
$_toast_logo_uploaded = "Logo uploaded successfully";
$_toast_logo_not_writable = "Web server cannot write to img/. Run: chown -R www-data:www-data img/ then chmod 775 img/ (match your PHP/nginx/apache user).";
$_toast_logo_upload_failed = "Failed to save logo. Check img/ ownership and ensure PHP GD is enabled for JPG/WEBP.";
$_toast_logo_file_locked = "Existing logo file cannot be overwritten. Run: chown www-data:www-data img/logo-*.png";
$_toast_logo_gd_missing = "JPG/WEBP upload requires PHP GD. Enable php-gd or upload a PNG file.";
$_toast_logo_not_image = "Selected file is not a valid image.";
$_toast_logo_too_large = "File is too large (max 600KB).";
$_toast_logo_bad_name = "File name must be %s";
$_toast_logo_select_file = "Please choose a logo file first.";
$_toast_logo_invalid_session = "Invalid session.";
$_toast_logo_invalid_request = "Invalid upload request. Reload the page and try again.";
$_toast_logo_invalid_type = "Unsupported file type. Use PNG, JPG, GIF, or WEBP.";
$_toast_logo_removed = "Logo deleted successfully";
$_toast_logo_remove_failed = "Logo could not be deleted";
$_uploading_logo = "Uploading logo...";
$_logo_upload_hint = "Upload any PNG or JPG image. Saved automatically as %s";
$_cpu_load = "CPU Load";
$_currency = "Currency";
$_dashboard = "Dashboard";
$_data_limit = "Data Limit";
$_date ="Date";
$_days = "days";
$_delete_data = "Delete data";
$_delete = "Delete";
$_dhcp_leases = "DHCP Leases";
$_dns_name = "DNS Name";
$_edit = "Edit";
$_edit_user = "Edit User";
$_end = "End";
$_expired = "Expired";
$_expired_mode = "Expired Mode";
$_extend_expired_date = "Extend Expired Date";
$_format_file_name = "Format file name";
$_free_hdd = "Free HDD";
$_free_memory = "Free Memory";
$_generate_code = "Generate Code";
$_generate = "Generate";
$_generate_user = "Generate User";
$_grace_period = "Grace Period";
$_help = "Help";
$_hosts = "Hosts";
$_hotspot_active = "Hotspot Active";
$_hotspot_cookies = "Cookies";
$_hotspot_log = "Hotspot Log";
$_hotspot_name = "Hotspot Name";
$_hotspot_users = "Hotspot User";
$_hours = "hours";
$_idle_timeout = "Idle Timeout";
$_income = "Income";
$_interface = "Interface";
$_ip_bindings = "IP Bindings";
$_last_generate = "Last Generate";
$_list_logo = "List Logo";
$_live_report = "Live Report";
$_loading = "Loading";
$_loading_interface = "Loading Interface";
$_loading_theme = "Loading theme";
$_lock_user = "Lock User";
$_log = "Log";
$_logout = "Logout";
$_messages = "Messages";
$_min = "min";
$_minutes = "minutes";
$_model = "Model";
$_name = "Name";
$_no = "No";
$_open = "Open";
$_package = "Package";
$_password = "Password";
$_please_login = "Please Login";
$_login_tenant_heading = "Workspace Login";
$_login_tenant_badge = "Workspace Tenant";
$_login_tenant_subtitle = "Manage routers, vouchers, and hotspot reports";
$_ppp_active = "PPP Active";
$_ppp_profiles = "PPP Profiles";
$_ppp_secrets = "PPP Secrets";
$_prefix = "Prefix";
$_price = "Price";
$_print_default = "Default";
$_print = "Print";
$_print_qr = "QR";
$_print_small = "Small";
$_processing = "Processing...";
$_profile = "Profile";
$_qty = "Qty";
$_quick_print = "Quick Print";
$_random = "Random";
$_readme = "Read Me";
$_reboot = "Are you sure to reboot";
$_reduce_expired_date = "Reduce Expired Date";
$_remove = "Remove";
$_report = "Report";
$_reset_start_date = "Reset Start Date";
$_resume = "Resume";
$_router_list = "Router List";
$_routers = "Routers";
$_router_hub = "All Routers";
$_account_settings = "Account Settings";
$_open_router = "Open";
$_online = "Online";
$_offline = "Offline";
$_unknown = "Unknown";
$_router_limit = "%d/%d routers";
$_router_limit_reached = "Router limit reached";
$_empty_routers = "No routers connected yet";
$_empty_routers_hint = "Add your first MikroTik router to get started.";
$_add_router_first = "Add First Router";
$_manage_routers = "Manage Routers";
$_router_name = "Router Name";
$_router_location = "Location";
$_router_slug_label = "Internal slug";
$_router_slug_hint = "Auto-generated from name. Edit only if needed.";
$_test_connection = "Test Connection";
$_connection_ok = "Connected";
$_connection_failed = "Connection failed";
$_test_required = "Test connection before saving";
$_wizard_step_identity = "Identity";
$_wizard_step_connection = "Connection";
$_wizard_step_hotspot = "Hotspot";
$_wizard_save_open = "Save & Open Dashboard";
$_skip_setup = "Configure later";
$_next = "Next";
$_back = "Back";
$_router_name_required = "Router name is required";
$_toast_router_added = "Router added successfully";
$_router_offline = "Router offline";
$_last_online = "Last online";
$_retry = "Retry";
$_storage_warning = "Storage {pct}% full";
$_storage_critical = "Storage critically low ({pct}% free)";
$_storage_banner = "Router storage is almost full. Delete old reports to prevent instability.";
$_storage_tiny_hint = "This router has very limited storage. Purge old reports regularly or use a model with more flash.";
$_storage_warn_hint = "Storage almost full. Delete old reports in Report menu, or connect USB storage.";
$_storage_critical_hint = "Storage critical — router may become unstable. Purge old reports immediately.";
$_purge_old_reports = "Delete reports older than {days} days";
$_purge_reports_confirm = "Delete {count} old report entries from this router?";
$_log_unavailable_storage = "Log unavailable — router storage is full";
$_mikrotik_api_guide = "MikroTik API setup guide";
$_save = "Save";
$_search = "Search";
$_sec = "sec";
$_seconds = "seconds";
$_selected = "Selected";
$_select_interface = "Select Interface";
$_selling_price = "Selling Price";
$_selling_report = "Selling Report";
$_send_to_WA = "Send to WhatsApp";
$_session_name = "Session Name";
$_session = "Session";
$_session_settings = "Session Settings";
$_settings = "Settings";
$_share = "Share";
$_show_all = "Show All";
$_shutdown = "Are you sure to shutdown";
$_start = "Start";
$_system_date_time = "System date & time";
$_system_off = "Shutdown";
$_system_reboot = "Reboot";
$_system_scheduler = "Scheduler";
$_system = "System";
$_template_editor = "Template Editor";
$_theme = "Theme";
$_this_month = "This month";
$_time_limit = "Time Limit";
$_time = "Time";
$_today = "Today";
$_total = "Total";
$_used = "Used";
$_free = "Free";
$_traffic_interface = "Traffic Interface";
$_traffic_monitor = "Traffic Monitor";
$_traffic = "Traffic";
$_upload = "Upload";
$_upload_logo = "Upload Logo";
$_uptime = "Uptime";
$_uptime_user = "Uptime";
$_user_length = "Name Length";
$_user_list = "User List";
$_user_log = "User Log";
$_user_mode = "User Mode";
$_user_name = "Username";
$_user_pass = "Username & Password";
$_user_profile_list = "Profile List";
$_user_profile = "User Profile";
$_users = "Users";
$_user_user = "Username = Password";
$_validity = "Validity";
$_voucher_code ="Voucher Code";
$_vouchers = "Vouchers";
$_yes = "Yes";





//details
$_format_time_limit = '
    Format '.$_time_limit.'.<br>
    [wdhm] Example : 30d = 30'.$_days.', 12h = 12'.$_hours.', 4w3d = 31'.$_days.'.
';
$_details_add_user = '
    '.$_add_user.' with '.$_time_limit.'.<br>
    Should '.$_time_limit.' < '.$_validity.'.
';

$_details_user_profile = '
'.$_expired_mode.' is the control for the hotspot user.<br>
Options : Remove, Notice, Remove & Record, Notice & Record.
<ul>
<li>Remove: User will be deleted when expires.</li>
<li>Notice: User will not deleted and get notification after user expiration.</li>
<li>Record: Save the price of each user login. To calculate total sales of hotspot users.</li>
</ul>
</p>
        
        <p>'.$_lock_user.' : Username can only be used on 1 device only.</p>
';

$_format_validity = '
Format '.$_validity.'<br>
[wdhm] Example : 30d = 30'.$_days.', 12h = 12'.$_hours.', 30m = 30'.$_minutes.'<br>
5'.$_hours.' 30'.$_minutes.' = 5h30m';

$_format_ip_binding = '
    Format Upload/Download Max Limit<br>
    [k / M] Contoh : 512k, 1500k, 1M<br><br>
    Format '.$_validity.'<br>
    [d] Example: 30d = 30'.$_days.'.<br>
';

$_help_report = '
<ul>
<li>Click CSV to download.</li>
<li>For filters per month, select Day and month, then click Filter.<br>
	<img width="70%" src="data:image/png;base64,">
	</li>
	<li>Filter based on '.$_prefix.', fill '.$_prefix.' in the search input, then click filter.</li>
	<li>Filter based on '.$_comment.', fill in !!'.$_comment.' in the  column, then click filter. Or click one of the comments. (MIKFAST Online).</li>
	<li>It is recommended to delete the sales report after download  the CSV report.</li>
	</ul>
';

$_delete_report = '
<ul>
		        <li>Deleting the Selling Report will delete the User Log as well. </li>
		        <li>It is recommended to download '.$_user_log.' first. </li>
		      </ul>
';

$_superadmin_panel = "Super Admin";
$_superadmin_login = "Super Admin Login";
$_superadmin_platform_title = "MIKFAST Platform";
$_superadmin_badge = "Platform Admin";
$_superadmin_subtitle = "Tenant management & platform console";
$_superadmin_login_button = "Enter Console";
$_login_not_superadmin_hint = "Not a platform admin? Open your tenant workspace subdomain instead.";
$_superadmin_not_configured = "Super-admin not set. Run: php scripts/superadmin-init.php superadmin your-password";
$_superadmin_login_url_hint = "Console: %s";
$_superadmin_change_password = "Change Password";
$_superadmin_new_password = "New password";
$_superadmin_pass_hint = "Stored encrypted in data/superadmin/credentials.json (folder data/ blocked from web).";
$_superadmin_create_tenant = "Create Tenant";
$_superadmin_slug = "Slug";
$_superadmin_domain = "Domain";
$_superadmin_label = "Label";
$_superadmin_tenants = "Tenants";
$_superadmin_active = "Active";
$_superadmin_suspended = "Suspended";
$_superadmin_total = "Total";
$_superadmin_tenant_count = "%d tenant(s)";
$_superadmin_storage = "Storage";
$_superadmin_empty = "No tenants yet.";
$_superadmin_empty_hint = "Use the form on the right to register your first tenant.";
$_superadmin_slug_hint = "https://{slug}.{domain}/admin.php?id=login";
$_superadmin_slug_help = "Lowercase subdomain identifier";
$_superadmin_search = "Search tenants...";
$_superadmin_open_tenant = "Open";
$_superadmin_account = "Account Settings";
$_superadmin_no_domain = "No domain set";
$_superadmin_no_results = "No tenants match your search.";
$_superadmin_unsuspend = "Activate";
$_superadmin_new_tenant = "New Tenant";
$_superadmin_nav_manage = "Manage";
$_superadmin_nav_system = "System";
$_superadmin_create_intro = "Register a new tenant workspace. The admin will log in from the subdomain you configure below.";
$_superadmin_create_help_title = "How it works";
$_superadmin_help_1 = "Pick a slug (subdomain) and base domain.";
$_superadmin_help_2 = "Set the tenant admin login credentials.";
$_superadmin_help_3 = "Share the login URL with your customer.";
$_superadmin_help_dns = "Ensure DNS wildcard (*.domain) points to this server.";
$_superadmin_console_host = "Console host";
$_superadmin_confirm_delete = "Delete tenant %s? This cannot be undone.";
$_superadmin_confirm_suspend = "Suspend tenant %s?";
$_superadmin_confirm_unsuspend = "Reactivate tenant %s?";
$_tenant_suspended = "This workspace is suspended. Contact support.";
$_login = "Login";
$_create = "Create";
$_status = "Status";
$_invalid_login = "Invalid username or password.";