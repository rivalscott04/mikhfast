<?php
/*
 * Account Settings — workspace admin credentials (router list moved to Router Hub).
 */
error_reporting(0);
	...
	<div class="col-12">
  	<div class="card">
  		<div class="card-header">
  			<h3 class="card-title"><i class="fa fa-user-circle"></i> <?= isset($_account_settings) ? $_account_settings : $_admin_settings ?> &nbsp; | &nbsp;&nbsp;<i onclick="location.reload();" class="fa fa-refresh pointer " title="Reload data"></i></h3>
  		</div>
      <div class="card-body">
<?php if (!empty($mikhmon_flash)) { ?>
        <div class="alert alert-success"><?= htmlspecialchars($mikhmon_flash, ENT_QUOTES, 'UTF-8'); ?></div>
<?php } ?>
        <p class="mm-sidenav-sub" style="margin:0 0 16px;">
          <a href="./admin.php?id=routers"><i class="fa fa-server"></i> <?= isset($_manage_routers) ? $_manage_routers : (isset($_router_hub) ? $_router_hub : 'Manage Routers') ?></a>
        </p>
        <div class="row">
			    <div class="col-6">
